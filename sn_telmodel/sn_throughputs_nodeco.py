...
class Throughputs(Telescope, Atmos_Transmission):
    ...
    def get_flux_sky(self, band, exptime, nexp):
        """
        Method to estimate the flux sky
        ...
        mag_sky = self.mag_sky(band)
        vv = self.mag_to_flux_e_sec(mag_sky,
                                    band,exptime, nexp)
        vv = vv[1]*photParams.platescale**2
        zpv = self.zp(band,exptime,nexp)
        vvb = 10**(-0.4*(mag_sky-zpv))
        vvb *= photParams.platescale**2
    
        self.data['flux_sky'][band] = vv
        
    def get_m5(self,band,exptime,nexp):
        """
        Method to estimate m5 value per band
    ...
    def get_Tb(self,band):
        """
        Method to estimate Tb
        throughput dependence

        Parameters
        ----------
        band : str
            filter to estimate Tb.
    ...
    def get_zp(self,band,exptime,nexp):
        """
        Method to estimate zerop point (mag)
        ...
        self.data['Skyb'][band] = Cte*np.power(Diameter/6.5, 2.)\
          ...
          * 10.**0.4*(25.-self.get_val_f('mag_sky',band))\
          * self.get_val_f('Sigmab',band)

        Zb = 181.8*np.power(Diameter/6.5, 2.)*self.get_val_f('Tb',band)
        mbZ = 25.+2.5*np.log10(Zb)
        self.data['zp'][band] = mbZ
        self.data['mbZ'][band] = mbZ
        
        
    def get_counts_zp(self,band,exptime,nexp):
        """
        Method to estimate count_zp and adu_zp
        ...
        flux0 = np.power(10., -0.4*self.get_val_f('mbZ',band))
        ...
        self.data['counts_zp'][band] = vval*photParams.gain  
        self.data['adu_zp'][band] = vval
    ...
    def get_val_fen(self,what,filtre, exptime, nexp):
        """
        Methoe to estimate quantitiy dependent on (band,exptime,nexp)
        m5,flux_sky,zp, count_zp,adu_zp,FWHMEff
        ...
        if (what not in self.data.keys()) or (filtre not in self.data[what].keys()):
            if what == 'adu_zp':
                what = 'counts_zp'
            to_eval = 'self.get_{}(\'{}\',{},{})'.format(what,filtre,exptime,nexp)
            exec(to_eval)
        
        return self.return_value(what, filtre)
        
        
    def get_val_f(self,what,filtre):
        """
        Method to estimate obs values ban dependent only
        Tb,mag_sky,sigmab
        ...
        if (what not in self.data.keys()) or (filtre not in self.data[what].keys()):
           to_eval = 'self.get_{}(\'{}\')'.format(what,filtre)
    ...
    def m5(self, band,exptime,nexp):
        """m5 accessor
        """
        return self.get_val_fen('m5',band,exptime,nexp)

    def Tb(self, band):
        """Tb accessor
        """
        
        return self.get_val_f('Tb',band)

    def mag_sky(self, band):
        """mag_sky accessor
        """
        
        return self.get_val_f('mag_sky', band)

    def flux_sky(self, band, exptime, nexp):
        """flux_sky accessor
        """

        
        return self.get_val_fen('flux_sky',band,exptime,nexp)
    ...
    def Sigmab(self,band):
        ...
        return self.get_val_f('Sigmab', band)

    def zp(self, band,exptime,nexp):
        """
        zp accessor
        ...
        return self.get_val_fen('zp', band,exptime,nexp)

    def counts_zp(self, band,exptime,nexp):
        """
        counts_zp accessor
        ...
        return self.get_val_fen('counts_zp', band,exptime,nexp)

    def adu_zp(self, band,exptime,nexp):
        """
        counts_zp accessor
        ...
        return self.get_val_fen('adu_zp', band,exptime,nexp)

    def FWHMeff(self, band):
        """
        FWHMeff accessor
        ...
        band: str
          filter
        """
        return self.get_val_f('FWHMeff', band)
